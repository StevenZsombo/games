...
class MM {
    ...
    static drawText(screen, txtorarr, rect, {
        ...
        screen.textAlign = textAlign
        const drawTextStartX = textAlign == "left" ? rect.x : textAlign == "center" ? (rect.x + rect.width / 2) : rect.x + rect.width
        screen.textBaseline = textBaseline
        ...
        return {
            ...
            getDifference(current) {
                ...
                for (const [key, val] of Object.entries(current))
                    if (val !== this._snapshot[key]) diff[key] = val
                return diff
            },
            _snapshotJSONableOnly: JSON.parse(JSON.stringify(original)),
            getDifferenceJSONableOnly(current) {
                const diff = {}
                for (const [key, val] of Object.entries(current))
                    if (JSON.stringify(val) !== JSON.stringify(this._snapshotJSONableOnly[key])) diff[key] = val
                return diff
            }
