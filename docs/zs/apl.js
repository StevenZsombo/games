// @ts-nocheck  // Disables TypeScript checking
...
`
  ...
  , asrt = x => {
    if (typeof x === 'function') { if (!x()) throw Error('assertion failed: ' + x) }
...
function Z(re, im) {
  asrt(typeof re === 'number'); asrt(typeof im === 'number' || im == null)
...
Z.div = (x, y) => {
  x = Zify(x); y = Zify(y); const d = y.re * y.re + y.im * y.im
...
Z.acos = x => {
  x = Zify(x); const r = Z.nit(Z.log(Z.add(x, Z.sqrt(Z.sub(Z.pow(x, 2), 1)))))
...
const LDC = 1, VEC = 2, GET = 3, SET = 4, MON = 5, DYA = 6, LAM = 7, RET = 8, POP = 9, SPL = 10, JEQ = 11, EMB = 12, CON = 13
  ...
  , vm = (b, h, p = 0, t = []) => {
    while (1) switch (b[p++]) {
      default: asrt(0) // b:bytecode,h:environment,p:program counter,t:stack
      case LDC: t.push(b[p++]); break
      case VEC: {
        let a = t.splice(t.length - b[p++]); for (let i = 0; i < a.length; i++)if (isSimple(a[i])) a[i] = unw(a[i])
        t.push(A(a)); break
      }
      ...
      case DYA: {
        let [y, f, x] = t.splice(-3)
        if (typeof f === 'function') {
          if (y instanceof Proc) y = toFn(y)
          if (x instanceof Proc) x = toFn(x)
      ...
      case SPL: {
        let n = b[p++], a = t[t.length - 1].a.slice().reverse(), a1 = Array(a.length)
      ...
      case CON: {
        let frm = h[h.length - 1], cont = { b, h: h.map(x => x.slice()), t: t.slice(0, frm[3]), p: frm[1].p + frm[1].size - 1 }
  ...
  , perv = (f1, f2) => { // pervasive f1:monad, f2:dyad
    ...
    let g2 = !f2 ? nyiErr : (x, y) => {
      switch ((!x.isA ? 10 : x.a.length === 1 ? 20 : 30) + (!y.isA ? 1 : y.a.length === 1 ? 2 : 3)) {
        ...
        case 33: {
          x.s.length !== y.s.length && rnkErr(); x.s != '' + y.s && lenErr()
...
voc['∘'] = conj((g, f) => {
  if (typeof f === 'function') {
    if (typeof g === 'function') { return (y, x) => f(g(y), x) }      // f∘g
    else { return (y, x) => { x == null || synErr(); return f(g, y) } }
  }  // f∘B
...
voc['∩'] = (y, x) => {
  x || nyiErr(); if (x.s.length > 1 || y.s.length > 1) rnkErr()
...
voc['⊤'] = (y, x) => {
  ...
  for (let i = 0; i < m; i++)for (let j = 0; j < y.a.length; j++) {
    ...
    for (let k = n - 1; k >= 0; k--) {
      let u = x.a[k * m + i]; r[(k * m + i) * y.a.length + j] = iszero(u) ? v : Z.mod(u, v)
...
const NOUN = 1, VRB = 2, ADV = 3, CNJ = 4
  ...
  , compile = (ast, o = {}) => {
    ...
    const gl = x => {
      switch (x[0]) {
        default: asrt(0) // categorise lambdas
    ...
    const rndr = x => {
      switch (x[0]) {
        default: asrt(0)
        case 'B': {
          if (x.length === 1) return [LDC, A.zld, RET]
        ...
        case 'X': {
          const s = x[1], vars = x.scp.v, v = vars['get_' + s]
          return s === '⍫' ? [CON] : v && v.g === VRB ? [LDC, A.zero, GET, v.d, v.i, MON] : [GET, vars[s].d, vars[s].i]
        }
        case '{': {
          const r = rndr(x[1]), lx = [LAM, r.length].concat(r); let f
          if (x.length === 2) { f = lx }
          else if (x.length === 3) {
            let y = rndr(x[2]), ly = [LAM, y.length].concat(y), v = x.scp.v['⍠']
        ...
        case 'T': {
          const u = x.scp.v._atop, v = x.scp.v._fork1, w = x.scp.v._fork2; let i = x.length - 1, r = rndr(x[i--])
    ...
    const rndrLHS = x => {
      switch (x[0]) {
        ...
        case '[': {
          const h = [], a = [], v = x.scp.v._amend // index assignment
