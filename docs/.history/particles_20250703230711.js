...
class Game {
	...
	initialize_more() {
		...
		this.shm = new SpatialHashGrid(this.WIDTH, this.HEIGHT, 10, 10)
		gd.boundingrects = []
		for (let p of gd.particles) {
			const b = p.boundingRect
			gd.boundingrects.push(b)
			this.shm.addClient(b)
		}
	...
	update_more(dt) {
		...
		gd.particles.forEach(x => x.color = "pink")
		this.shm.findNear(gd.first.boundingRect).forEach(x => x.color = "blue")
		...
		for (let p of gd.particles) {
			...
			this.shm.updateClient(p.boundingRect)
		}
		for (let p of gd.particles) {
			const friends = this.shm.findNear(p)
			for (let q of friends) {
				if (p.collidecirc(q)) {
					Particle.collidePhysics(p, q)
					let stillcollides = false //DISABLE FOR NO CHEATS, WILL STICK THO
					gd.cheatcount--
	...
	draw_more(screen) {
		...
		//for (let p of gd.particles) { MM.drawText(this.screen, p.shmindices, p.x, p.y, "8x Times") }
