...
class Game {
    ...
    initialize_more() {
        this.framerate.isRunning = false
        this.clickables.length = 0
        this.grid = this.rect.copy.
            deflate(20, 10).
            splitRow(3, 2, 10).
            map(x => x.deflate(0, 5)).
            map(x => Button.fromRect(x, { outline: 2 }))
        this.add_clickable(this.grid[0])
        this.grid[0].txt = "you ahve batteries"
        this.batteries = this.grid[1].splitCol(...Array(12).fill(1)).map(Button.fromRect).forEach(b => {
            b.deflate(5, 5)
        })
