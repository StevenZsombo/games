...
class Game {
    ...
    initialize_more() {
        ...
        this.buts = butGrid.flat().map(x => (Button.fromRect.bind(x))(x))


        this.buts.forEach(x => { x.deflate(8, 8) })
        this.buts.forEach(this.add_clickable.bind(this))
        let a = Date.now() / 1000
        let b = 0
        ...
        this.buts.forEach(b => {
            b.on_click = () => {
                this.animator.add_anim(
                    b, 800, "scaleToFactor",
                    { scaleFactor: .1, on_end: function () { this.obj.stretch(.1, .1) } })
