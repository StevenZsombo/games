...
class Rect {
	...
	get centerX() { this.x + this.width / 2 }
	get centerY() { this.y + this.height / 2 }
	get center() {
		return {
...
class Button extends Clickable {
	...
	draw(screen) {
		if (this.visible) {
			if (this.deg || this.rad) {
				screen.save()
				this.rad ??= this.deg * ONEDEG
			...
			if (this.rad || this.deg) {
				screen.restore() //started above
			}
