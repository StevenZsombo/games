...
class RectRotated extends Rect {
	constructor(rect) {
	...
	static rotatePointAround(x, y, a, b, rad) {
		const [dx, dy] = [x - a, y - b]
		const r = RectRotated.rotatePointAroundOrigin(dx, dy, rad)
		return ({
			x: r.x + a,
			y: r.y + b
		})
	...
	rotateAround(u, w, rad, doNotAdjustFacing = false) {
		this.rad += rad
		this.centeratV(RectRotated.rotatePointAround(this.cx, this.cy, u, w, rad))
	}
}
