...
class Game {
    ...
    initialize_more() {
        ...
        this.swap = (bu, bw, animate = 300, dontCheck = false) => {
            if (bu === bw) {
                return
            }
            const [i, j] = bu.tag
        ...
        this.shuffle = (animate = 100, times = 30) => {
            for (let i = 0; i < times; i++) {
                ...
                setTimeout(doit.bind(this), i * 100)
            }
        }
        ...
        this.animator.add_sequence(
            new Anim(this.empty, 200, "delay", { on_end: () => this.empty.resize(1, 1) }),
